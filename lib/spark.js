...
Object.defineProperties(Spark.prototype, {
  ...
  find: {
     ...
     * @param {Object} predicate - the example of the query that you want to run, for example if you had an object {name: "billy"}, this will
     * use the object billy to compare the entities with.
     * @param {Object} options
     */
    value: function (collection, predicate, options) {
      ...
      self._checkConfiguration();
      var url = self._apiUrl('/_api/simple/by-example');

      options.collection = collection;
      options.example = predicate;
  ...
  findById: {
    ...
    value: function (collection, id) {
      ...
      if (!collection || collection.length == 0) throw new Errors.SparkError('Collection is required');
      if (!id || id.length == 0 && _.isNumber(id)) throw new Errors.SparkError('Id is required, and must be a number');

      self._checkConfiguration();

      var deferred = Q.defer();
      var documentHandle = collection + '/' + id;
      var url = self._apiUrl('/_api/document/' + documentHandle);
      ...
      request({
        ...
        if (err || body && body.errorMessage)
          if (body && body.code === '404') throw new Errors.SparkError(body.errorMessage);
          else deferred.reject(new Error(body.errorMessage));
